...
// ตรวจสอบว่ามีข้อมูลอีเมลใหม่ที่ต้องการยืนยันหรือไม่
if (!isset($_SESSION['verify_new_email'])) {
    $_SESSION['error_message'] = 'ไม่พบข้อมูลอีเมลที่ต้องการยืนยัน';
...
$new_email = $_SESSION['verify_new_email'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['verify_otp'])) {
        ...
        if (empty($otp_code)) {
            ...
            if (verifyOTP($database, $user_type, $user_id, 'email', $otp_code)) {
                // อัปเดตอีเมลใหม่ในฐานข้อมูลตามประเภทผู้ใช้
                if ($user_type === 'company') {
                    $sql = "UPDATE companies SET contact_email = ?, updated_at = NOW() WHERE company_id = ?";
                } elseif ($user_type === 'student') {
                    $sql = "UPDATE students SET email = ?, updated_at = NOW() WHERE student_id = ?";
                } elseif ($user_type === 'admin') {
                    $sql = "UPDATE admins SET email = ?, updated_at = NOW() WHERE admin_id = ?";
                }
                ...
                if ($result) {
                    ...
                    unset($_SESSION['verify_new_email']);
                    
                    $_SESSION['success_message'] = 'อัปเดตอีเมลสำเร็จ';
                    ...
                    $_SESSION['error_message'] = 'เกิดข้อผิดพลาดในการอัปเดตอีเมล: ' . $database->errorInfo()[2];
                }
            } else {
    ...
    elseif (isset($_POST['resend_otp'])) {
        ...
        $stmt->execute([$user_type, $user_id, 'email']);
        
        // สร้าง OTP ใหม่
        $otp = createOTP($database, $user_type, $user_id, 'email', $new_email);
    ...
    // ถ้าต้องการยกเลิกการเปลี่ยนอีเมล
...
// ดึงข้อมูลผู้ใช้เพื่อแสดงอีเมลปัจจุบัน
$current_email = '';
if ($user_type === 'company') {
    $company = getCompanyProfile($database, $user_id);
    $current_email = $company['contact_email'] ?? '';
} elseif ($user_type === 'student') {
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">ยืนยันการเปลี่ยนอีเมล</h4>
                </div>
                <div class="card-body">
                    ...
                    <div class="alert alert-info">
                        <p><strong>อีเมลปัจจุบัน:</strong> <?php echo htmlspecialchars($current_email); ?></p>
                        <p><strong>อีเมลใหม่:</strong> <?php echo htmlspecialchars($new_email); ?></p>
                    </div>
                    
                    <p>รหัส OTP ได้ถูกส่งไปยังอีเมล <?php echo htmlspecialchars($new_email); ?> กรุณากรอกรหัสเพื่อยืนยันการเปลี่ยนแปลง</p>
